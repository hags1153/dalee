...
export type Stats = {
  ...
  lastCircuitDay: number; totalScore: number; gamesPlayed: number;
};

const emptyStats = (): Stats => ({ streak: 0, maxStreak: 0, circuitsCompleted: 0, lastCircuitDay: -999, totalScore: 0, gamesPlayed: 0 });
...
export async function commitCircuit(day: number, d: DayState): Promise<Stats> {
  ...
  stats.totalScore += circuitScore(d);
